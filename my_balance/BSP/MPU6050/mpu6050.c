...
sum=0;
for(i=0;i<10;i++){	//求当前数组的合，再取平均�?Find the sum of the current array and then take the average
...
Output  : none
函数功能：设�?MPU6050 加速度计的最大量�?
入口参数：range：加速度最大量程编�?
返回  值：�?
**************************************************************************/
//#define MPU6050_ACCEL_FS_2          0x00  		//===最大量�?-2G Maximum range +-2G
//#define MPU6050_ACCEL_FS_4          0x01			//===最大量�?-4G Maximum range +-4G
//#define MPU6050_ACCEL_FS_8          0x02			//===最大量�?-8G Maximum range +-8G
//#define MPU6050_ACCEL_FS_16         0x03			//===最大量�?-16G Maximum range +-16G
...
Input   : enable�?，sleep�?，work�?
Output  : none
函数功能：设�?MPU6050 是否进入睡眠模式
入口参数：enable�?，睡觉；0，工作；
返回  值：�?
**************************************************************************/
...
Input   : none
Output  : 1：Connected�?：Not connected
...
Input   : enable�?，yes�?;not
Output  : none
函数功能：设�?MPU6050 是否为AUX I2C线的主机
入口参数：enable�?，是�?：否
返回  值：�?
**************************************************************************/
...
void MPU6050_initialize(void) {
    ...
    MPU6050_setFullScaleAccelRange(MPU6050_ACCEL_FS_2);	//加速度度最大量�?+-2G Maximum acceleration range +-2G
    MPU6050_setSleepEnabled(0); //进入工作状�?Enter working state
	  MPU6050_setI2CMasterModeEnabled(0);	 //不让MPU6050 控制AUXI2C Prevent MPU6050 from controlling AUXI2C
	  MPU6050_setI2CBypassEnabled(0);	 //主控制器的I2C�?MPU6050的AUXI2C	直通关�?I2C of the host controller and AUXI2C of MPU6050 are directly closed
}
...
{	   
	  ...
	  Temp=(I2C_ReadOneByte(devAddr,MPU6050_RA_TEMP_OUT_H)<<8)+I2C_ReadOneByte(devAddr,MPU6050_RA_TEMP_OUT_L);
		...
		Temp=(36.53+Temp/340)*10;	  //温度放大十倍存�?Temperature magnified ten times for storage
	  return (int)Temp;
}
//------------------End of File----------------------------
