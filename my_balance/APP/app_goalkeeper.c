...
/*
 * 三位置守门：小车初始位于球门中央。
 * 球较远或保持在中央区域时原地观察；蓝球验证通过且面积达到
 * 门槛后锁定方向，走一段固定弧线，随后站住。
 */
float Goalkeeper_Gyro_Kd = 1.2f;          /* 弧线转向阻尼；甩动时增大。 */
float Goalkeeper_Lane_Speed = 85.0f;      /* 速度PI命令量，不是实际车速上限。 */
float Goalkeeper_Lane_Turn_PWM = 910.0f;  /* 随前进速度同比提高，保持弧线曲率。 */
...
#define GOALKEEPER_TRIGGER_AREA_X10               10U /* 10表示全屏面积1.0%。 */
#define GOALKEEPER_TRIGGER_CONFIDENCE             35U /* 单帧触发的最低置信度。 */
#define GOALKEEPER_LANE_CONFIRM_FRAMES             1U /* 满足门槛后立即锁定。 */
#define GOALKEEPER_SIDE_TURN_TICKS               22U /* 提速后保持弧线距离。 */
#define GOALKEEPER_SIDE_STRAIGHT_TICKS            9U /* 提速后保持直行距离。 */
 ...
 */
#define GOALKEEPER_ACCEL_STEP                  7.50f
#define GOALKEEPER_DECEL_STEP                  1.13f /* 同步缩短减速滑行距离。 */

#define GOALKEEPER_TURN_SLEW_PWM             147.0f /* 按速度比例提高转向建立速率。 */
